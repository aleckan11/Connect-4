...
def put_piece(board, max_rows, column, player):
    ...
    -1
    """
    #indexing starts at zero, rows start at 1
    index_row = max_rows-1
    while index_row >= 0 and get_piece(board, index_row, column) != "-":
        index_row -= 1
    if index_row >= 0:
        with_piece = replace_elem(board[index_row], column, player)
        board = replace_elem(board, index_row, with_piece)
    return index_row, board
